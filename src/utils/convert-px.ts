...
if (canUseDom) {
  ...
  tenPxTester.className = 'fgdm-px-tester'
  tenPxTester.style.setProperty('--size', '10')
  document.body.appendChild(tenPxTester)
  tester = document.createElement('div')
  tester.className = 'fgdm-px-tester'
  document.body.appendChild(tester)
  if (isDev) {
    if (window.getComputedStyle(tester).position !== 'fixed') {
      devError(
        'Global',
        'The px tester is not rendering properly. Please make sure you have imported `fg-mobile/es/global`.'
      )
    }
  }
}
